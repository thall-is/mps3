...
inline void IRAM_ATTR DSPProcessor::processStereo(float &L, float &R) {
    ...
    // Modo Fone de Ouvido Estereo Normal (L e R Full-Range, sem divisor de frequencia)
    if (m_bassBoostEnabled) {
        // Aplica o bass shelf individualmente em cada canal mantendo o estereo puro
        L = m_bassShelfL.process(L) * DSP_BASS_GAIN_BOOST;
        R = m_bassShelfR.process(R) * DSP_BASS_GAIN_BOOST;
    }

    // Inversao de canais L/R (se configurado pelo usuario)
