...
void NativeA2DPSink::av_hdl_a2d_evt(uint16_t event, void *p_param) {
    ...
    case ESP_A2D_AUDIO_CFG_EVT: {
        ...
        if (audio_type == ESP_A2D_MCT_SBC) {
            ...
            ch = ((raw[2] & 0x08) || ((raw[2] >> 2) & 0x03) == 1) ? 1 : 2;
            bits = 16;
            ESP_LOGI(BT_AV_TAG, "Detected AAC codec sr=%u bits=0x%03X ch=%u raw=%02X %02X %02X %02X %02X %02X",
                     sr, sampleBits, ch,
                     raw[0], raw[1], raw[2], raw[3], raw[4], raw[5]);
        } else if (audio_type == ESP_A2D_MCT_NON_A2DP) {
            // Vendor codec (aptX, LDAC, etc.): parse raw CIE bytes
            // LDAC layout in 8-byte raw CIE: vendorId[0-3], codecId[4-5], sampleRate[6], channelMode[7]
