...
// Protocolo de controle entre a placa principal (ESP32-S3) e este
...
// (se o CRC nao bater, o parser descarta 1 byte e tenta resincronizar no
// proximo 0xA5, entao um 0xA5 espurio dentro de um payload corrompido
// no maximo causa descarte do quadro atual, nao trava o link).
...
typedef enum {
    ...
    UART_CMD_SET_QUALITY      = 0x15, // payload: 1 byte (0=HQ,1=SQ,2=MQ) - ver ldac_enc_quality_t
    UART_CMD_SET_SAMPLE_RATE  = 0x16, // payload: 4 bytes (uint32 LE) - Hz, deve bater com o que sai no I2S
...
// Enviada via UART_CMD_SET_CODEC. O companheiro salva na NVS e
// aplica na proxima conexao (ou reconexao se ja conectado).
typedef enum {
...
// Chamado pela tarefa interna do uart_ctrl quando um quadro completo e
// valido (CRC ok) chega. Roda no contexto da tarefa uart_ctrl - se o
// tratamento for demorado, poste pra outra fila em vez de bloquear aqui.
...
typedef struct {
    ...
    int tx_gpio;
    int rx_gpio;
    int baud_rate;         // sugestao: 115200 - da' folga pro PCM nao passar por aqui (so' controle)
    uart_ctrl_rx_cb_t rx_cb;
    void *user_ctx;
} uart_ctrl_config_t;

// Inicializa o driver UART e sobe a tarefa de recepcao. Retorna
// ESP_OK/ESP_FAIL (ver esp_err.h - incluido indiretamente via driver/uart.h
// na implementacao).
int uart_ctrl_init(const uart_ctrl_config_t *cfg);

// Monta e envia um quadro. Thread-safe (protegido por mutex interno).
// Retorna true se conseguiu enfileirar a escrita.
bool uart_ctrl_send(uart_ctrl_cmd_t cmd, const uint8_t *payload, size_t len);

// Atalhos pros eventos mais comuns que o companheiro envia.
