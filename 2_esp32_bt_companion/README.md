...
## 📚 Créditos de Engenharia Reversa e Protocolos

A viabilização de codecs avançados de alta definição (Sony LDAC e Qualcomm aptX) e a injeção de múltiplos SEPs na pilha Bluedroid do ESP32 foram possíveis graças aos esforços de engenharia reversa e dissecação de protocolos da comunidade open-source:
* **Wireshark e Linux BlueZ**: Engenharia reversa dos pacotes AVDTP e mapeamento dos Codec Information Elements (CIE) para Vendor IDs proprietários (`0x00012D` Sony, `0x00004F`/`0x0000D7` Qualcomm).
* **ValdikSS & Pesquisadores Bluetooth**: Descoberta do cabeçalho de payload LDAC de 1 byte (`num_frames`) sobre RTP e técnicas de injeção de codecs de alta definição em pilhas Bluetooth embarcadas.
* **Android Open Source Project (AOSP)**: Pelo código-fonte de referência do encoder LDAC em ponto fixo (`libldac`) e implementação de referência em `a2dp_vendor_ldac.cc`.
* **WillyBilly06**: Pelo trabalho pioneiro demonstrando patches Bluedroid e operação multi-codec no ESP32 clássico sem necessidade de PSRAM externa.
